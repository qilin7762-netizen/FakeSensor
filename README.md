...
1. 安装模块
2. 在Magisk等Root管理器添加本模块的su执行权限
3. 打开 LSPosed 管理器，启用本模块，勾选目标 App（必选系统框架）
4. 重启系统使模块生效
5. 打开 FakeSensor App
6. 勾选需要伪造的传感器（至少一个）
7. 点击 **静态模拟** 或 **动态模拟**
...
通过滑块实时调节各轴数值，拖动时实时保存配置（`simulate=false`）。目标 App 读取传感器时将始终返回滑块设定的固定值，适合需要精确控制传感器数值的场景。

### 动态模拟

基于预设场景实时生成变化的数据（`simulate=true`）：
...
- ⚠️ **本模块仅供学习和开发测试使用**，不得用于任何欺骗或违反应用服务条款的目的
- ⚠️ **仅在 MIUI 13 (Android 12) 测试过**，其他版本兼容性未知，欢迎反馈
- ⚠️ **不得用于金融应用反欺诈绕过**，否则可能违反当地法律
- ⚠️ **使用者需自行承担所有法律责任**，开发者不承担因本项目使用造成的任何直接或间接损害
